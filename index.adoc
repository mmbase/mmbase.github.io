...
The example webapp is dockerized, and most things are still working:
...
Remember that the default username/password you can use is admin/admin2k

image:example-webapp.png[]

This is running memory only. To have it persistent, it can e.g. be done like so:

[source, bash]
----
curl -L https://tinyurl.com/r3j8zwpj  -o docker-compose.yml && DOCKER_USER=$(id -u):$(id -g) docker-compose up
----
This employs a link:https://raw.githubusercontent.com/mmbase/mmbase/refs/heads/MMBase-1_9/example-webapp/docker-compose.yml[docker-compose.yml] file, which uses volumes to persist data and work directories, and a bit of configuration to make HSQLDB write to CSV files.
...
== This page
This page is hosted at https://github.com/mmbase/mmbase.github.io[github pages], so currently are just static pages. We're using adoc. The original documentation of MMBase was in docbook XML's, I'll try to get them published here too somehow.

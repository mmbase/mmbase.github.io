...
At the time (around ~ 2010 or so) we were actually busy with an MMBase 2.0, which would have involved portal functionality and such. We may try to release some of it after all. We also started to split up the mono-repository of MMBase 1.9 in several distinct github repository like:
